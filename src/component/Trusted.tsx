...
//         <h1 className="text-2xl font-bold text-[#0D0D0D] ">Trusted by Industry Leaders</h1>
...
//               style={{
//                 marginTop: openIndex === index ? '-4px' : '0', // Adjust position to touch the border above
    ...
    <div className="mx-auto px-4 pb-16 pt-16 max-w-6xl sm:pb-24 sm:pt-24">
      ...
      {faqs.map((faq, index) => (
        ...
        >
          <button
            onClick={() => toggleQuestion(index)}
            className="w-full font-semibold text-left p-3 sm:p-4 text-gray-800 flex justify-between items-center text-sm sm:text-base lg:text-lg"
          >
            <span>{faq.question}</span>
            <svg
              xmlns="http://www.w3.org/2000/svg"
              ...
              className="transform transition-transform"
              style={{
                marginTop: openIndex === index ? '0' : '-8px', // Move the arrow closer to the border when closed
                transform: openIndex === index ? 'rotate(180deg)' : 'rotate(0deg)',
              }}
            >
              <path d="M213.66,101.66l-80,80a8,8,0,0,1-11.32,0l-80-80A8,8,0,0,1,53.66,90.34L128,164.69l74.34-74.35a8,8,0,0,1,11.32,11.32Z"></path>
